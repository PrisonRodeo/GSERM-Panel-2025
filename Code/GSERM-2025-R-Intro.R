...
# it has 20 observations (rows) and
...
setwd("~/Dropbox (Personal)/GSERM/Panel-2025/Data")
...
# you have the data stored "locally" in a 
# directory named like the one above.) 
#
# We can also read the same data directly
# from the web, using a URL for the file.
# We can do this using the same command
# ("read.csv") because it can also read
# directly from an ASCII-encoded .CSV file
# that is at a URL:

SCOTUS <- read.csv("https://github.com/PrisonRodeo/GSERM-Panel-2025/raw/main/Data/SCOTUS-votes.csv")
...
#
# Once we've read in the data, we can do 
...
# which tells us that Justice Gorsuch was 
# appointed by a Republican. This is 
# indexing useful for a lot of things; for 
# example, we can use it along with 
# conditional statements to subset the
# data, like this:
...
# appointees (that is, the observations
# where the logical statement "GOP==1"
# is true)." We can see that the command
# worked:
...
# Also, here we've used a "formula" to denote
# the variables we want plotted; a formula
# generally has the form "Y~X" (or "Y~X+Z",
# etc.) with the tilde (~) denoting that the
# thing to the left of it (here, Y) is a 
# function of the thing(s) to the right of
# it (X).
#
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
